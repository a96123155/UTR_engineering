...
fold = 5
#label = 'rl'
label = 'mRNA_Half_life'
r2_results = np.zeros(fold)
pearsonr_results = np.zeros(fold)
sp_cor_results = np.zeros(fold)
cell_line = 'Hela'  #hek muscle pc3 HEK293 Hela
#data_file = '../data/mRNA_HL_Hela+HEK_3UTR300.csv'
data_file = '../data/mRNA_HL_Hela+HEK_5UTR200.csv'
...
df[label] = df[label].apply(np.log2)
...
for train_index,test_index in KF.split(df):
    ...
    seq_e_train = train.one_hot_encode(e_train, col='utr', reverse=True , seq_len=seq_length)
    seq_e_test = train.one_hot_encode(e_test, col='utr', reverse=True, seq_len=seq_length)
    ...
    e_pred = test_data(df=e_test, model=model, test_seq=seq_e_test, obs_col=label)
    r = r2(e_pred[label], e_pred['pred'])
    pearson_r = pearsonr(e_pred[label], e_pred['pred'])
    sp_cor = spearmanr(e_pred[label], e_pred['pred'])
