...
#added on 2022.7.31, previous used all sample
df = df.iloc[:280000]
#scale_utrs = df[:40000]
scaler = preprocessing.StandardScaler()
scaler.fit(df['rl'].values.reshape(-1,1))
scaler_dic['egfp'] = scaler
scaler_dic['egfp0'] = scaler
scaler_dic['egfp1'] = scaler
scaler_dic['egfp2'] = scaler


#df = pd.read_csv('../data/RSV UTR Seeds.csv')
#df = pd.read_csv('../data/5UTR from paper.csv')
df = pd.read_csv('../data/5UTR_seqs_211.txt')
...
seq_e_test = one_hot_encode(e_test, col='5\' UTR', seq_len=seq_length)
...
model_dic = {
    ...
    'egfp0': load_model('../modeling/saved_models/main_MRL_model.hdf5'),
    'egfp1': load_model('../modeling/saved_models/Optimus_model1.hdf5'),
    'egfp2': load_model('../modeling/saved_models/Optimus_model2.hdf5'),
...
#df.to_csv('../data/5UTR from paper Predicted.csv')
df.to_csv('../data/5UTR_seqs_211_prediced.txt')
