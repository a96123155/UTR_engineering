...
def train_model(x, y, border_mode='same', inp_len=50, nodes=40, layers=3, filter_len=8, nbr_filters=120,
    ...
    model = Sequential()
    if layers >= 1:
        model.add(LSTM(units=32, input_shape=(inp_len, 4), return_sequences=True))
    if layers >= 2:
...
model.save('./saved_models/'+ cell_line + '-len'+str(seq_length)+ '-Sp'+ str(round(sp_cor_results.max(), 3)) + '-' + theTime + 'RNN.hdf5')
#pred_df.to_csv('../data/Muscle_prediction_3RNN.csv')
