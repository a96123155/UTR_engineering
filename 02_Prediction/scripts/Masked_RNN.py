import keras
from keras.layers import Input, Dense, Conv1D, GlobalMaxPooling1D, GlobalAveragePooling1D, Dropout, Concatenate, Lambda, \
    Flatten, ZeroPadding1D, MaxPooling1D, BatchNormalization, ThresholdedReLU, Masking, Add, LSTM, SpatialDropout1D
...
def convolve_and_mask(conv_features, pad_mask, n_filters, kernel_size, suffix, prefix="",
    ...
    conv_features = Lambda(apply_pad_mask, name=prefix + "apply_pad_mask_" + suffix)(
        [conv_features, pad_mask])  # Mask padding
    if batchnorm:
...
def inception_block(conv_features, pad_mask, n_filters, suffix, prefix=""):
    conv_features_3 = convolve_and_mask(conv_features, pad_mask, n_filters[0], kernel_size=3,
                                        suffix="incept3_" + suffix, prefix=prefix)
    conv_features_5 = convolve_and_mask(conv_features, pad_mask, n_filters[1], kernel_size=5,
                                        suffix="incept5_" + suffix, prefix=prefix)
    conv_features_7 = convolve_and_mask(conv_features, pad_mask, n_filters[2], kernel_size=7,
                                        suffix="incept7_" + suffix, prefix=prefix)
